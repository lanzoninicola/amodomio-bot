...
import bot from "../libs/whatsapp-wppconnect";
...
import { RenderPageOptions, htmlGenerator } from "../libs/html-generator";
import whatsappWppConnect from "../libs/whatsapp-wppconnect";
...
class WhatsappBotController {
  ...
  async test() {
    ...
    for (let message of messages) {
      ...
      let resposta = await this.processMessage(
        false,
        msg.body,
        msg.from,
        msg.from,
  ...
  async procAjuda(chatOrGroupId: string): Promise<string> {
    await bot.sendInteractiveMessage(chatOrGroupId, {
  ...
  async procPDF(
    ...
    try {
      ...
      await this.sendImage(chatOrGroupId, message, base64file, "", messageId);
      return "-1";
    } catch (e) {
  ...
  async sendMessage(
    ...
    try {
      await bot.sendMessage(chatOrGroupId, message, quotedMsg, continueTyping);
    } catch (e) {
  ...
  async sendFile(
    ...
    try {
      await bot.sendFileFromBase64(to, message, base64File, fileName);
    } catch (e) {
  ...
  async checkChatExists(chatOrGroupId: string) {
    try {
      return await bot.checkChatExists(chatOrGroupId);
    } catch (e) {
  ...
  //   }
  //   return (await bot.getGrupos(telefone)).map((grupo) => ({
